...
class GradientDescentSolver(Solver):
    ...
    def get_updates_without_momentum(self, loss_tensor, param_vars):
        ...
        for i in range(len(param_vars)):
            w = param_vars[i]
            new_w = w - self.learning_rate * grad_tensors[i]
    ...
    def get_updates_with_momentum(self, loss_tensor, param_vars):
        ...
        for i in range(len(param_vars)):
            ...
            updates.extend([(vel_vars[i], new_vel), (param_vars[i], new_weight)])
    ...
    def solve(self, input_train_data, target_train_data, input_val_data, target_val_data, model, callback=None):
        ...
        for iter_ in range(self.iterations):
            train_loss, weights = session.run([loss_tensor, update_ops], feed_dict={target_ph: target_train_data, model.input_ph: input_train_data})
            val_loss = session.run(loss_tensor, feed_dict={target_ph: target_val_data, model.input_ph: input_val_data})
...
class StochasticGradientDescentSolver(GradientDescentSolver):
    ...
    def solve(self, input_train_data, target_train_data, input_val_data, target_val_data, model, callback=None):
        ...
        trainDataGenerator = MinibatchIndefinitelyGenerator(train_data, 1, self.shuffle)
        valDataGenerator   = MinibatchIndefinitelyGenerator(val_data, 1, self.shuffle)
        ...
        for iter_ in xrange(self.iterations):
            ...
            next_train_data, next_val_data = trainDataGenerator.next(), valDataGenerator.next()

            train_loss, weights = session.run([loss_tensor, update_ops], feed_dict={model.input_ph: next_train_data[0], target_ph: next_train_data[1]})
            val_loss = session.run(loss_tensor, feed_dict={model.input_ph: next_val_data[0], target_ph: next_val_data[1]})
...
class MinibatchStochasticGradientDescentSolver(GradientDescentSolver):
    ...
    def solve(self, input_train_data, target_train_data, input_val_data, target_val_data, model, callback=None):
        ...
        train_losses = []
        val_losses = []
        trainDataGenerator = MinibatchIndefinitelyGenerator(train_data, self.batch_size, self.shuffle)
        valDataGenerator   = MinibatchIndefinitelyGenerator(val_data, self.batch_size, self.shuffle)
        for iter_ in range(self.iterations):
