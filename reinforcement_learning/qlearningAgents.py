...
class QLearningAgent(ReinforcementAgent):
    ...
    def getQValuesOfState(self, state):
        if state not in self.qValues:
            self.qValues[state] = {action: 0.0 for action in self.getLegalActions(state)}
    ...
    def getQValue(self, state, action):
        ...
        """
        qValues = self.getQValuesOfState(state)
    ...
    def computeValueFromQValues(self, state):
        ...
        "*** YOUR CODE HERE ***"
        qValues = self.getQValuesOfState(state)
        return max([self.getQValue(state, action) for action in qValues]) if qValues else 0.0
    ...
    def computeActionFromQValues(self, state):
        ...
        bestActions = [action for action in qValues if self.getQValue(state, action) == maxQ]
    ...
    def update(self, state, action, nextState, reward):
        ...
        """
        "*** YOUR CODE HERE ***"
        _ = self.getQValuesOfState(state) # populate state dictionary if doesn't exist already
